...
Original version: 30 September, 2024
...
For our demonstration, we will use the Spearman correlation and choose 10 assays
that overlap between the AlphaMissense and ProteinGym databases for 
benchmarking.
...
Subset the AlphaMissense scores, DMS scores, and the model metrics to these
chosen assays.
...
we will need to calculate this for AlphaMissense now. Start by merging the DMS 
and AlphaMissense tables by matching protein mutants.
...
`merge_am_dms`shows the AlphaMissense and DMS scores for protein mutants that 
...
To prepare for plotting, we'll pivot our dataset to long format and reorder our 
models from the highest mean Spearman to the lowest.
...
For the purposes of our example, select the top 10 performing models.
...
For the Spearman correlation, AlphaMissense performed the best across the DMS 
assays selected. For the purposes of this vignette, we demonstrate with just 10 
assays, but it is possible to apply this framework across all 217 DMS assays
available in ProteinGym.
